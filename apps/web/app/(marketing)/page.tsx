...
import {
  CircuitBoard, ShieldCheck, Zap, Library,
  ArrowRight, Brain, ExternalLink,
  Building2, Target, GraduationCap, Cpu,
  Map, Smartphone,
} from 'lucide-react';
    ...
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-cx-border bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
        ...
        <div className="hidden items-center gap-6 md:flex">
          ...
          <a href="#preview" className="font-mono text-xs text-cx-muted transition-colors hover:text-cyan-400">preview</a>
        </div>
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <Link href="/auth" className="font-mono text-xs text-cx-muted transition-colors hover:text-foreground/80">
            sign in
          </Link>
          <Link
            href="/auth"
...
function MockArchMap() {
  const nodes = [
    ...
    { id: 'comp',   label: 'FolderIngestor', x: '38%',  y: '45%', color: '#4ade80' },
    ...
    <div className="relative h-44 w-full overflow-hidden rounded border border-cx-border bg-cx-surface">
      ...
      {nodes.map((n) => (
        <div
          ...
          style={{ left: n.x, top: n.y, borderColor: n.color + '50', transform: 'translate(-50%,-50%)' }}
        >
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-24">
        ...
        >
          ...
          <div>
            ...
            <motion.p variants={fadeUp} className="mb-8 font-mono text-sm leading-7 text-cx-muted">
              CortexPath is your architectural intelligence system. Mirror your local code to the cloud,
              generate AI logic summaries, and audit your system design in real-time.
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="mt-8 flex items-center gap-6">
              {[
                ...
                <div key={stat.value} className="text-center">
                  <p className="font-mono text-sm font-semibold text-cyan-400">{stat.value}</p>
                  <p className="font-mono text-[10px] text-cx-muted/70">{stat.label}</p>
                </div>
              ))}
            </motion.div>
          </div>
            ...
            <div className="rounded-xl border border-cx-border bg-cx-surface p-4 backdrop-blur-sm">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono text-[10px] text-cx-muted">cortex://architecture-map</span>
                </div>
                <div className="flex gap-1.5">
                  {['bg-red-500', 'bg-yellow-500', 'bg-green-500'].map((c) => (
                    <span key={c} className={`h-2 w-2 rounded-full ${c} opacity-60`} />
                  ))}
                </div>
              </div>
              <MockArchMap />
              <div className="mt-3 flex items-center justify-between">
                <span className="font-mono text-[10px] text-cx-muted/70">5 nodes · 4 edges</span>
                <span className="font-mono text-[10px] text-green-400">● live sync</span>
              </div>
            </div>
          </motion.div>
        </motion.div>
  ...
  return (
    <section id="features" className="relative bg-background py-24">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        >
          <p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-cyan-700">core capabilities</p>
          <h2 className="font-sans text-3xl font-bold text-foreground">Three pillars. One system.</h2>
        </motion.div>
        ...
        >
          {PILLARS.map(({ icon: Icon, color, border, glow, title, badge, desc }) => (
            <motion.div
              key={title}
              variants={fadeUp}
              className={`group rounded-xl border ${border} bg-cx-surface p-6 backdrop-blur-sm transition-all ${glow}`}
            >
              <div className={`mb-4 flex w-fit rounded-lg border ${border} bg-cx-surface p-2.5`}>
                <Icon size={18} className={color} />
              </div>
              <span className={`mb-2 block font-mono text-[10px] uppercase tracking-widest ${color}`}>
                {badge}
              </span>
              <h3 className="mb-2 font-sans text-lg font-semibold text-foreground">{title}</h3>
              <p className="font-mono text-xs leading-6 text-cx-muted">{desc}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
function MentorMomentSection() {
  return (
    <section id="preview" className="relative overflow-hidden bg-background py-24">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <motion.div
          ...
          className="mb-14"
        >
          <p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-cyan-700">interactive preview</p>
          <h2 className="font-sans text-3xl font-bold text-foreground">The Mentor Moment.</h2>
          <p className="mt-2 font-mono text-sm text-cx-muted">
            Ingest your entire codebase once — then click any file in the tree to stream a structured AI audit rooted in real imports, exports, and dependency context.
          </p>
        </motion.div>
          ...
          <motion.div variants={fadeUp}>
            <div className="rounded-xl border border-cx-border bg-cx-surface overflow-hidden">
              <div className="flex items-center gap-2 border-b border-cx-border px-4 py-2.5">
                <span className="font-mono text-[10px] text-cx-muted/70">hooks/useDebounce.ts</span>
                <span className="ml-auto rounded bg-cyan-950/50 px-1.5 py-0.5 font-mono text-[9px] text-cyan-400">LIVE</span>
              </div>
          ...
          <motion.div variants={fadeUp}>
            <div className="rounded-xl border border-cx-border bg-cx-surface overflow-hidden">
              <div className="flex items-center gap-2 border-b border-cx-border px-4 py-2.5">
                ...
                <span className="font-mono text-[10px] text-cx-muted">cortex audit</span>
                <span className="ml-auto font-mono text-[9px] text-green-400">● complete</span>
              </div>
              <div className="space-y-4 p-5">
                {AUDIT_ITEMS.map(({ Icon, color, label, text }) => (
                  <div key={label} className="flex gap-3">
                    ...
                    <div>
                      <p className={`mb-0.5 font-mono text-[9px] uppercase tracking-widest ${color}`}>{label}</p>
                      <p className="font-mono text-xs leading-5 text-cx-muted">{text}</p>
                    </div>
                  </div>
                ))}
              </div>
    ...
    <section id="mobile" className="relative bg-background py-24">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="flex justify-center">
            <div className="relative h-[420px] w-[210px] rounded-[2.2rem] border-[6px] border-cx-border bg-background shadow-2xl">
              ...
              <div className="mt-6 space-y-3 p-4">
                <p className="font-mono text-[9px] uppercase tracking-widest text-cyan-700">blast radius</p>
                {['api/route.ts', 'hooks/useIngestor', 'components/FilePanel'].map((f, i) => (
                  <div key={f} className="flex items-center gap-2 rounded border border-cx-border bg-cx-surface px-2.5 py-1.5">
                    <span className={`h-1.5 w-1.5 rounded-full ${i === 0 ? 'bg-red-400' : 'bg-cx-muted/40'}`} />
                    <span className="font-mono text-[9px] text-cx-muted">{f}</span>
                  </div>
                ))}
                <div className="mt-4 rounded border border-purple-800/40 bg-purple-950/20 p-3">
                  <p className="font-mono text-[9px] text-purple-400 uppercase tracking-widest mb-1">mentor challenge</p>
                  <p className="font-mono text-[9px] text-cx-muted leading-4">Add error boundaries at integration points...</p>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          {/* Copy */}
          <motion.div variants={fadeUp}>
            <p className="mb-3 font-mono text-[10px] uppercase tracking-widest text-cyan-700">mobile librarian</p>
            ...
            <p className="mb-6 font-mono text-sm leading-7 text-cx-muted">
              The Expo Librarian app gives lead engineers instant access to code logic, Blast Radius
              risks, and mentorship challenges — anywhere, without touching a laptop.
            </p>
            <ul className="space-y-3">
              {[
                { icon: Library,     text: 'Browse summaries for every ingested file' },
                { icon: Target,      text: 'Blast Radius alerts before you push' },
                { icon: GraduationCap, text: 'Mentor challenges delivered as push notifications' },
              ].map(({ icon: Icon, text }) => (
                <li key={text} className="flex items-start gap-3 font-mono text-xs text-cx-muted">
      ...
      <div className="mx-auto max-w-6xl px-6">
        <div className="flex flex-col items-start justify-between gap-8 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-2">
            {[
              ...
              { label: 'github', href: 'https://github.com/JasDevPH/cortex-path' },
        ...
        <div className="mt-10 border-t border-cx-border pt-6 text-center">
          <p className="font-mono text-[10px] text-cx-muted/50">
            © {new Date().getFullYear()} CortexPath · Built with Next.js, Groq, Neon, and pgvector
          </p>
        </div>
      </div>
    </footer>
